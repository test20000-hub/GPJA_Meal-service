import json
import os
from datetime import date, timedelta
from pathlib import Path

import requests

API_KEY = os.environ["NEIS_API_KEY"]
BASE = "https://open.neis.go.kr/hub"
SCHOOL_NAME = "군포중앙고등학교"
OUT = Path("data/meals.json")


def get_json(endpoint, params):
    response = requests.get(f"{BASE}/{endpoint}", params={"KEY": API_KEY, "Type": "json", **params}, timeout=30)
    response.raise_for_status()
    data = response.json()
    if "RESULT" in data:
        result = data["RESULT"]
        raise RuntimeError(f"NEIS {result.get('CODE')}: {result.get('MESSAGE')}")
    return data


def find_school():
    data = get_json("schoolInfo", {"pIndex": 1, "pSize": 1000, "SCHUL_NM": SCHOOL_NAME})
    rows = data.get("schoolInfo", [])
    if len(rows) < 2 or not rows[1].get("row"):
        raise RuntimeError(f"학교를 찾을 수 없습니다: {SCHOOL_NAME}")
    matches = [r for r in rows[1]["row"] if r.get("SCHUL_NM") == SCHOOL_NAME]
    if not matches:
        matches = rows[1]["row"]
    school = matches[0]
    return school["ATPT_OFCDC_SC_CODE"], school["SD_SCHUL_CODE"]


def fetch_meals(office_code, school_code, start, end):
    data = get_json("mealServiceDietInfo", {
        "pIndex": 1,
        "pSize": 1000,
        "ATPT_OFCDC_SC_CODE": office_code,
        "SD_SCHUL_CODE": school_code,
        "MLSV_FROM_YMD": start.strftime("%Y%m%d"),
        "MLSV_TO_YMD": end.strftime("%Y%m%d"),
    })
    rows = data.get("mealServiceDietInfo", [])
    if len(rows) < 2:
        return {}
    meals = {}
    for row in rows[1].get("row", []):
        if row.get("MMEAL_SC_CODE") != "2":
            continue
        key = row["MLSV_YMD"]
        menu = [x.strip() for x in row.get("DDISH_NM", "").replace("<br/>", "\n").replace("<br>", "\n").split("\n") if x.strip()]
        value = {"menu": menu}
        if row.get("CAL_INFO"):
            try:
                value["kcal"] = float(row["CAL_INFO"].replace(" Kcal", "").strip())
            except ValueError:
                pass
        meals[key[:4] + "-" + key[4:6] + "-" + key[6:8]] = value
    return meals


def main():
    today = date.today()
    end = today + timedelta(days=30)
    office_code, school_code = find_school()
    meals = fetch_meals(office_code, school_code, today, end)
    meals = {k: v for k, v in meals.items() if date.fromisoformat(k).weekday() < 5}
    if not meals:
        raise RuntimeError("NEIS에서 급식 데이터가 0건 반환되었습니다.")
    payload = {
        "school": SCHOOL_NAME,
        "source": "https://open.neis.go.kr/portal/mainPage.do",
        "provider": "NEIS",
        "updatedAt": today.isoformat(),
        "range": {"from": today.isoformat(), "to": end.isoformat(), "weekdaysOnly": True},
        "meals": meals,
    }
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"NEIS: generated {len(meals)} weekday meal dates ({today} ~ {end})")


if __name__ == "__main__":
    main()
