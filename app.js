...
function endOfWeek(date) { return moveDate(startOfWeek(date), 6); }
...
function render() {
  ...
  if (meal) {
    (meal.menu || []).forEach(item => {
      const li = document.createElement('li'); li.textContent = item; $('menuList').appendChild(li);
    });
  }
  renderWeek();
}
...
function renderWeek() {
  const start = startOfWeek(selected); const end = endOfWeek(selected);
  ...
  const root = $('weekList'); root.innerHTML = '';
  for (let i=0;i<7;i++) {
    const d = moveDate(start, i), key = toKey(d), meal = meals[key];
    const item = document.createElement('button'); item.className = `week-item ${key===toKey(selected)?'active':''}`;
    const date = document.createElement('span'); date.className='week-date'; date.textContent = `${d.getMonth()+1}/${d.getDate()} ${d.toLocaleDateString('ko-KR',{weekday:'short'})}`;
    const menu = document.createElement('span'); menu.className='week-menu'; menu.textContent = meal ? (meal.menu || []).join(' · ') : '급식 없음';
    item.append(date, menu); item.onclick=()=>{selected=d; render();}; root.appendChild(item);
  }
}

$('prevBtn').onclick = () => { selected = moveDate(selected,-1); render(); };
$('nextBtn').onclick = () => { selected = moveDate(selected,1); render(); };
$('todayBtn').onclick = () => { selected = new Date(); render(); };

fetch(DATA_URL, { cache: 'no-store' })
  .then(r => r.json())
  .then(data => { meals = data.meals || {}; $('updatedAt').textContent = data.updatedAt ? `데이터 업데이트: ${data.updatedAt}` : ''; render(); })
  .catch(() => { $('mealCard').classList.remove('loading'); $('mealTitle').textContent='급식 데이터를 불러오지 못했습니다'; $('emptyMessage').classList.add('hidden'); });
